...
def read_seqs_from_file(seqs_file, seq_column_name='Sequence'):
    """input_file must be txt or csv, or xlsx"""
    seqs_file = Path(seqs_file)
    assert seqs_file.exists(), f"input_file not exists: {seqs_file}"
    if seqs_file.suffix == ".txt":
        ...
        raise ValueError(
            f"input_file must be txt or csv, or xlsx, but got {seqs_file.suffix}"
        )
    _seqs = []
    for seq in seqs:
        if seq:
            _seqs.append(seq)
    assert _seqs, f"seqs is empty in {seqs_file}"
    logger.info('seqs num %s', len(_seqs))
    return _seqs
