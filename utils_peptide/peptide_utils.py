...
def is_natural_and_variable_length(seq, min_len=5, max_len=15):
    """ Includes the end. """
    if seq and len(seq) >= min_len and len(seq) <= max_len and is_natural_only_upper(seq):
        return True
    return False
...
def is_natural_and_max_length_30(seq):
    if seq and len(seq) >= LEAST_SEQ_LENGTH and len(seq) <= 30 and is_natural_only_upper(seq):
        return True
    return False

def is_natural_and_2_50_length(seq):
    """ 2~50, inclusive, is_natural_only_upper """
    if seq and len(seq) >= LEAST_SEQ_LENGTH and len(seq) <= MAX_SEQ_LEN and is_natural_only_upper(seq):
        return True
    return False
...
def cal_novelty(generated_seqs, all_pos_neg_seqs):
    novel_seq = []
    ...
    logger.info('len(novel_seq) %s, novelty_ratio %s, all available positive and negative seq num %s',
                len(novel_seq), novelty_ratio, len(all_pos_neg_seqs))
    return novel_seq, novelty_ratio


def cal_uniqueness(seqs):
    ...
    logger.info('len(unique_seqs_dict) %s, uniqueness_ratio %.1f%%, all input seq num %s, ',
                len(unique_seqs_dict), uniqueness_ratio, len(seqs))
...
def get_concentration(species):
    ...
    if len(concentration_str) == 0:
        ...
        concentration = convert_plus_minus_as_only_plus(concentrations[0]) + convert_plus_minus_as_only_plus(concentrations[1])
        concentration /= 2
    else:
